...
* * Análise de classificação de dados (profunda) (varredura de mapa e classificação)*: Varre a classificação BlueXP  usando protocolos padrão e permissão somente leitura em todos os seus ambientes. Os arquivos selecionados são abertos e digitalizados para dados confidenciais relacionados a negócios, informações privadas e problemas relacionados ao ransomware.
+
