...
[role="lead"]
Conclua algumas etapas para instalar a classificação BlueXP  em um host Linux em um site local que não tenha acesso à Internet - também conhecido como _modo privado_. Este tipo de instalação é perfeito para seus sites seguros.
...
|===
Se você estiver usando vários hosts de classificação BlueXP  para fornecer poder de processamento adicional para verificar suas fontes de dados, precisará ativar portas/protocolos adicionais. link:task-deploy-compliance-dark-site.html["Consulte os requisitos de porta adicionais"].
...
image:diagram_deploy_onprem_multi_host_no_internet.png["Um diagrama mostrando a localização das fontes de dados que você pode verificar ao usar várias instâncias de classificação do BlueXP  implantadas no local sem acesso à Internet."]
