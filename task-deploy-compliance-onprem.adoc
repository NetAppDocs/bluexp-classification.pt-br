...
Para configurações muito grandes em que você digitalizará petabytes de dados, nas versões 1,30 e anteriores, é possível incluir vários hosts para fornecer poder de processamento adicional. Ao usar vários sistemas host, o sistema primário é chamado de nó _Manager_, e os sistemas adicionais que fornecem poder de processamento extra são chamados de _nó do scanner_.
...
Você também pode link:task-deploy-compliance-dark-site.html["Instale a classificação BlueXP  em um site local que não tenha acesso à Internet"]para sites completamente seguros.


NOTE: Para versões antigas 1,30 e anteriores, para adicionar nós de scanner, link:task-deploy-add-scanner-nodes.html["Adicione nós do scanner a uma implantação existente"]consulte .
...
Para configurações muito grandes em que você digitalizará petabytes de dados, você pode incluir vários hosts para fornecer poder de processamento adicional. Saiba mais link:task-deploy-multi-host-install-dark-site.html> sobre a instalação em vários hosts para configurações grandes.

image:diagram_deploy_onprem_multi_host_internet.png["Um diagrama mostrando a localização das fontes de dados que você pode verificar ao usar várias instâncias de classificação do BlueXP  implantadas no local com acesso à Internet."]
...
https://youtu.be/rFpmekdbORc["Assista a este vídeo"^] Para ver como instalar a classificação BlueXP .
...
+
Observe que o instalador executa uma pré-verificação para garantir que seus requisitos de sistema e rede estejam em vigor para uma instalação bem-sucedida. https://youtu.be/_RCYpuLXiV0["Assista a este vídeo"^] compreender as mensagens de pré-verificação e implicações.
